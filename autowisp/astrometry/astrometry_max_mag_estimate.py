...
from autowisp.processing_steps.solve_astrometry import add_anet_cmdline_args, prepare_configuration
from autowisp.catalog import create_catalog_file, read_catalog_file

###########################
# 1. why shouldn't we grab the fov estimate directly from solved ast.net? instead of setting it up
# in the config manually while the value from astrometry.net is a good one?
# if so, how can we get it from local astrometry.net? (online version gives it in the response page)
# 2. pylint complains about the eval() function, and I dont't know how to fix it.

###########################

# pylint falsely complains about the units module
...
def read_dr_file(
    ...
    Args:
        ...
        cmdline_args (dict or Namespace): Command line arguments or a dictionary of
        configuration parameters.

    Returns:
        field_corr (np.ndarray): Initial correlation data from astrometry.net.
        dr_df (pd.DataFrame): DataFrame containing the extracted sources from the DR file.
    """
    ...
    with DataReductionFile(dr_path, "r") as dr_file:
        ...
        xy_extracted_struct = np.zeros(xy_extracted.shape[0], dtype=[("x", "f8"), ("y", "f8")])
...
def match_sources(field_corr, dr_df):
    ...
    field_corr_native = field_corr.astype(
        field_corr.dtype.newbyteorder('='))
    corr_df = pd.DataFrame(field_corr_native,
    ...
    print(f"Out of {n_extracted} extracted sources, corr_matched is found as: \n {corr_matched} ")

    return corr_matched, n_extracted

# pylint: disable=too-many-locals
def query_gaia(
    corr_matched, n_extracted,
    image_scale_factor,
    ...
    Returns:
        corr_matched_sorted (pd.DataFrame): DataFrame containing the matched sources
        sorted by magnitude with Gaia phot_g_mean_mag.
    """
    ...
    height = eval(frame_fov_estimate[0])
    width = eval(frame_fov_estimate[1])

    create_catalog_file(
        'cat.fits', overwrite=True,
        ra=ra_center*units.deg, dec=dec_center*units.deg,
        width=width, height=height,
        magnitude_expression='phot_g_mean_mag',
        epoch=2025*units.yr,
        magnitude_limit=20.0,   # Need to have a limit for WISPGaia.query_brightness_limited()
    ...
    def make_coords(df, ra_col, dec_col):
        return SkyCoord(ra=df[ra_col].values * units.deg, dec=df[dec_col].values * units.deg)
    ...
    print(
        f"Gaia G magnitude of the 10% brightest sources:"
        f"{round(gaia_results.iloc[int(n_extracted*1.1)]['phot_g_mean_mag'], 2)}"
    )

    # Assign Gaia G magnitude to sources
    corr_matched['phot_g_mean_mag'] = np.nan
    corr_matched.loc[match_mask, 'phot_g_mean_mag'] = (
        gaia_results['phot_g_mean_mag'].iloc[idx[match_mask]]
    )
