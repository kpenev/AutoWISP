...
def get_typical_star(dr_fnames,
    ...
    source_averaged = pandas.DataFrame(
        getattr(get_matched_sources(fname, dr_path_substitutions),
                source_average)()
...
def get_center_background(dr_file,
                          header,
    ...
    source_positions = {
        coord: dr_file.get_dataset('srcproj.columns',
...
def get_frame_merit_info(dr_fname,
    ...
    with DataReductionFile(dr_fname, 'r') as dr_file:
        ...
        psf_map_predictors = get_predictors_and_weights(
            ...
            dr_file.get_attribute('srcextract.psf_map.cfg.terms',
                                  **dr_path_substitutions),
            None
        )[0]
        psf_map_coefficients = dr_file.get_dataset('srcextract.psf_map',
                                                   **dr_path_substitutions)
        for psf_param, map_coef in zip(
            dr_file.get_attribute('srcextract.psf_map.cfg.psf_params',
                                  **dr_path_substitutions),
