...
def _parse_phot_column(colname):
    ...
    for phot_quantity in _PHOT_QUANTITIES:
        is_phot = (
            is_phot
            or
            (colname.find('_' + phot_quantity + '_') > 0)
        )
    if not is_phot:
        return None, -1
        ...
        if phot_quantity is None:
            if not colname.startswith('hat_id_'):
                dtype.append((
                    colname,
                    (
                        'string_' if source_data[colname].dtype.kind == 'O'
...
def get_magfit_sources(data_reduction_file,
    ...
    phot_index = {quantity: [0] * len(magfit_iterations)
                  for quantity in _PHOT_QUANTITIES}
