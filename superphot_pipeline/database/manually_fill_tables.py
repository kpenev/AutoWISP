...
from data_model import ObservingSession, Target
...
with Session.begin() as db_session:
    ...
    telescope_type = TelescopeType(make="Rokinon",
                                   model="85M-C",
                                   version="",
                                   f_ratio=1.4,
                                   focal_length=85,
    ...
    telescope = Telescope(serial_number = "00000",
                          notes = "",
                          timestamp = datetime.now()
                          )

    observer = Observer(name = "PAN001",
                        email = "wtylergee@gmail.com",
                        phone = "",
                        timestamp = datetime.now()
                        )

    observatory = Observatory(latitude = 19.54,
                              longitude = -155.58,
                              altitude = 3400.0,
    ...
    observing_session_1 = ObservingSession(start_time = datetime(2018, 12, 28, 12, 4, 26),
                                           end_time = datetime(2019, 1, 24, 14, 29, 28),
                                           notes = "PAN001_14d3bd_20190124T115445",
                                           timestamp = datetime.now()
                                           )

    observing_session_2 = ObservingSession(start_time=datetime(2018, 12, 28, 9, 40, 19),
    ...
    db_session.add_all([camera_type,
                        mount_type,
                        telescope_type,
                        observer,
                        observatory,
